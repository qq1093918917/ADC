...
void LCD_ShowChar(u16 x,u16 y,u8 num,u8 size,u8 mode)//mode=0时 有文本框，mode=1时 直接叠加
{  							  
    u8 temp,t1,t;
	u16 y0=y;
...
{         	
	...
	for(t=0;t<len;t++)
	{
		...
		if(enshow==0&&t<(len-1))
		{
			if(temp==0)
			{
				LCD_ShowChar(x+(size/2)*t,y,' ',size,0);//mode=0时 有文本框，mode=1时 直接叠加
				continue;
			}else enshow=1; 
		 	 
		}
	 	LCD_ShowChar(x+(size/2)*t,y,temp+'0',size,0); //mode=0时 有文本框，mode=1时 直接叠加
	}
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
...
{         
    ...
    {       
        ...
        LCD_ShowChar(x,y,*p,size,1);//mode=0时 有文本框，mode=1时 直接叠加
        x+=size/2;
        p++;
    }  
}
void LCD_Showbmp16(u16 x,u16 y, const unsigned char *p,u16 l,u16 w)
{
  	int i; 
	unsigned char picH,picL; 
	LCD_Set_Window(x,y,x+l-1,y+w-1);//窗口设置
	for(i=0;i<l*w;i++)
	{	
	 	picL=*(p+i*2);	//数据低位在前
		picH=*(p+i*2+1);				
