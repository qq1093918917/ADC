...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)//接收的数据已装载到BUFF，校验数据完整或正确
{
	//while(1);
//	HAL_UART_Receive(&huart1,USART_RX_BUF,1,1000); 
...
void USART1_Data_Real(u8 buf[],u8 len)
{
  u8 num,i;
	...
	{
		if(USART_RX_BUF[i]==' ') num++;
		else
		{
			a[num]=a[num]*10+(USART_RX_BUF[i]-'0');
		}
	}
	len=num;
	printf("总共输入%d个数值,分别如下：",num+1);
	printf("\r\n\r\n");//插入换行
	USART_RX_STA=0;
	for(i=0;i<=num;i++)//缓冲初始化
	{
		buf[i]=a[i];
		printf("%d ",buf[i]);
		a[i]=0;
